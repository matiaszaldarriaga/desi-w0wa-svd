...
Since all probes measure the same \czero direction (Section~\ref{sec:universal_c0}), we can compare their \czero measurements directly. The tension formula (Eq.~\ref{eq:tension}) gives four independent measurements of the same quantity.
...
\tablecomments{Values in parentheses use the Planck 2018 \LCDM chain as the CMB reference instead of ACT DR6.}
\tablehead{
...
The most striking feature of Table~\ref{tab:c0_tensions} is that BAO alone prefers lower \omh (positive \czero tension at $+2.2\sigma$), while all three SN datasets prefer higher \omh or are consistent with zero. Union3 ($-1.7\sigma$) and Pantheon+ ($-1.1\sigma$) both prefer higher \omh; DES-Dovekie ($-0.8\sigma$) is consistent with null. If the signal were purely cosmological, all probes should agree on both sign and magnitude (when interpreted as a shift in cosmological parameters). The sign pattern---BAO positive, SN negative or null---suggests the tension may originate in the CMB--BAO system rather than in new low-$z$ physics.

An important point ot remember when interpreting Figure~\ref{fig:c0_gaussians}: the \czero normalization differs between probes. A unit shift in BAO \czero and a unit shift in SN \czero both correspond to $1\sigma$ of their respective measurement errors, but these translate to different shifts in \omh (see Section~\ref{sec:money_plot}). The figure is designed to visualize \emph{tensions}, not to compare \omh values directly. 
...
Since \czero is a near-perfect proxy for \omh (Eq.~\ref{eq:c0_formula}), we can convert each probe's \czero measurement to an implied \omh value. This should be interpreted as the value of \omh that is implied by the data if one follows the CBM degeneracy direction. The conversion inverts the $\beta$-vector formula, setting \obh and \thetastar to their chain mean values:
\begin{equation}
...
Figure~\ref{fig:omegamh2_money} shows the result. BAO provides the tightest constraint ($\sigma_\mathrm{total} = 0.00087$, combining measurement and marginalization), exceeding the CMB itself. The SN total errors ($\sigma_\mathrm{total} \approx 0.004$) are $\sim 5\times$ larger.
...
For distances, lower \omh (positive \czero) produces smaller $D_V/\rs$ ratios at all BAO redshifts (Figure~\ref{fig:c0_families}, left panel): the slower high-$z$ expansion reduces distances, while the higher $H_0$ reduces $\rs$-normalized quantities further. For SN, the same physics manifests as a tilt in $\mu(z)$: positive \czero shifts distance moduli downward (brighter supernovae) at low $z$ and upward at high $z$. The opposite signs of the BAO and SN tensions thus correspond to opposite distance residual patterns visible in the raw data (Figures~\ref{fig:dv_ratios} and~\ref{fig:sn_datasets}). Note that when making this plots we just varied \omh while keeping \obh and \thetastar fixed for simplicity. Varying \obh and \thetastar along the degeneracy does not change things qualitatively.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_hz_omh2_variation.pdf}
...
\subsection{Central Finding Within \LCDM}
\label{sec:central_finding_lcdm}

Within \LCDM, there is a tension between DESI and the CMB which lives in the direction we are calling  the \omh direction---the worst-constrained CMB combination parameter for low-$z$ expansion history. No other direction is measurable: $\sigma_{c_1} = 0.06$ for BAO, $\approx 0.001$ for SN (Table~\ref{tab:sigma_c}). This fact is not particularly surprising. The \czero tension is real---BAO genuinely prefers a different \omh than the CMB---but mild. The absence of SN confirmation---all three SN datasets are mild or null---cautions against a simple cosmological interpretation, but of course the SN error bars are significantly larger.  
In the next section, we extend the analysis to \wowa dark energy and ask whether this tension opens genuinely new directions, or whether the DESI \wowa preference is simply \czero tension repackaged.
